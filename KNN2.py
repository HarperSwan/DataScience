...
#    mailDataset = pd.read_csv(csvFilePath, header = None)  # names=names,
    ...
    #pop ispam
    spam = mailDataset.pop('isSpam')

    dataLabels = mailDataset.iloc[:, csvValuesColumnNumber].values  
    
    X_train, X_test = train_test_split(dataFieldsValues, test_size=0.2) # test_size = 1 - train_size
    Xspam_train, Xspam_test = train_test_split(spam, test_size=0.2)
    ...
    y_predict = predictWith("KNN", X_train, X_test, Xspam_train, Xspam_test)
    
    print(confusion_matrix(Xspam_test, y_predict))
    predictedRatio = np.mean(y_predict != Xspam_test)
    
    print("predictedRatio = %f" %(predictedRatio))
    print(classification_report(Xspam_test, y_predict))
...
def predictWith(algoName, X_train, X_test, y_train, y_test) : 
    ...
    if (algoName == "KNN") : # K-Nearest Neighbors
        ...
        classifier = KNeighborsClassifier(n_neighbors=150)  #♪ with 4 neighboors closest
